...
public class IronextractorUpdateTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		playerConfig = new File((FMLPaths.GAMEDIR.get().toString() + "/config/"), File.separator + (owner + ".json"));
		{
			try {
				...
				if (mainObject.get("Iron").getAsDouble() > 10 && 64 > new Object() {
					...
					mainObject.addProperty("Iron", (mainObject.get("Iron").getAsDouble() - 1));
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
