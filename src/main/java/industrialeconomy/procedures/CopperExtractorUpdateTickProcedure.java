...
public class CopperExtractorUpdateTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		playerConfig = new File((FMLPaths.GAMEDIR.get().toString() + "/config/"), File.separator + (owner + ".json"));
		{
			try {
				...
				if (mainObject.get("Copper").getAsDouble() > 10 && 64 > new Object() {
					...
					mainObject.addProperty("Copper", (mainObject.get("Copper").getAsDouble() - 1));
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
