...
public class CateriumExtractorUpdateTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		playerConfig = new File((FMLPaths.GAMEDIR.get().toString() + "/config/"), File.separator + (owner + ".json"));
		{
			try {
				...
				if (mainObject.get("Caterium").getAsDouble() > 10 && 64 > new Object() {
				...
				}.getAmount(world, new BlockPos(x, y, z), 0) && true == (new Object() {
				...
				}.getValue(world, new BlockPos(x, y, z), "Working"))) {
					{
					...
					mainObject.addProperty("Caterium", (mainObject.get("Caterium").getAsDouble() - 1));
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
