...
public class PocketchestMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public PocketchestMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		super(IndustrialEconomyModMenus.POCKETCHEST.get(), id);
	...
	public ItemStack quickMoveStack(Player playerIn, int index) {
		...
		if (slot != null && slot.hasItem()) {
			...
			if (index < 147) {
				if (!this.moveItemStackTo(itemstack1, 147, this.slots.size(), true))
					return ItemStack.EMPTY;
				slot.onQuickCraft(itemstack1, itemstack);
			} else if (!this.moveItemStackTo(itemstack1, 0, 147, false)) {
				if (index < 147 + 27) {
					if (!this.moveItemStackTo(itemstack1, 147 + 27, this.slots.size(), true))
						return ItemStack.EMPTY;
				} else {
					if (!this.moveItemStackTo(itemstack1, 147, 147 + 27, false))
						return ItemStack.EMPTY;
				}
				return ItemStack.EMPTY;
			}
